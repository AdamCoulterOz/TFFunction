...
namespace TF
{
	public class Terraform
	{
		...
		{
			return await command("plan", log, withVars: true);
		}

		public async Task<string> Apply(ILogger log)
		{
			return await command("apply", log, withVars: true, autoApprove: true);
		}

		public async Task<string> Destroy(ILogger log)
		{
			return await command("destroy", log, autoApprove: true);
		}

		private async Task<string> command(string action, ILogger log, bool autoApprove = false, bool withVars = false)
		{
			...
			log.LogInformation(JsonSerializer.Serialize(command.EnvironmentVariables));
			if (DryRun) { return command.ToString(); }
